...
const LandingBanner = () => {
  const slides = [
    ...
    { 
      ...
      details: [
        'Candy, Pearl, & Metallic Finishes',
        'Custom Airbrushing & Graphics',
        'Matte & Satin Vinyl Wraps'
      ]
    },
    { 
      src: img4, 
      ...
      details: [
        'Aluminum & Carbon Fiber Repair',
        'Certified High-End Vehicle Technicians',
        'Discreet & Secure Facility'
      ]
    },
    { 
      src: img5, 
    ...
    { 
      ...
      details: [
        ...
        'Maintains Original Factory Paint',
                ...
                >
                    ...
                    <div className="absolute inset-0 pt-24 pb-12 md:pb-24">
                        <div className="w-full max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col justify-center">
                            <div className={`max-w-md md:max-w-xl lg:max-w-3xl transition-all duration-1000 ease-in-out ${index === currentIndex ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tighter text-white leading-tight drop-shadow-lg">
                                    {slide.headline}
                                </h1>
                                <div className="w-20 h-1.5 bg-red-600 my-5 sm:my-6"></div>
                                ...
                                <div className="mt-6 space-y-3">
                                  {slide.details.map((detail, i) => (
                                    <div key={i} className="flex items-center gap-3">
                                      <svg className="flex-shrink-0 w-5 h-5 text-red-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
            {slides.map((_, index) => (
                <button
                    ...
                    className={`w-3 h-3 rounded-full transition-all duration-300 ${currentIndex === index ? 'bg-red-600 w-6' : 'bg-white/50 hover:bg-white/80'}`}
                ></button>
            ))}
        </div>
    </section>
