...
const Lightbox = ({ images, activeIndex, onClose, onNext, onPrev }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-lg z-50 flex items-center justify-center animate-fade-in p-4">
      ...
      <div className="relative w-full h-full max-w-6xl max-h-[80vh] flex items-center justify-center">
        <img src={image.src} alt={image.alt} className="max-w-full max-h-full object-contain animate-fade-in-up" />
      </div>
    </div>
  );
};
...
const GalleryPage = () => {
  ...
  const nextItem = () => setLightboxIndex((prev) => (prev + 1) % galleryImages.length);
  const prevItem = () => setLightboxIndex((prev) => (prev - 1 + galleryImages.length) % galleryImages.length);
        ...
        <section className="py-16 sm:py-24 text-center">
          <div className="max-w-4xl mx-auto px-4">
            ...
            <p className="mt-6 text-lg text-gray-400 max-w-2xl mx-auto">
              Explore a collection of our finest work. Each project showcases our commitment to five-star quality, from minor touch-ups to major restorations.
            </p>
          </div>
        </section>
        ...
        <section className="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 pb-16 sm:pb-24">
          <div className="columns-2 md:columns-3 lg:columns-4 gap-4 sm:gap-6">
            {galleryImages.map((image, index) => (
              <div
                key={index}
                className="mb-4 sm:mb-6 break-inside-avoid cursor-pointer group relative overflow-hidden rounded-lg"
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                  <p className="text-white text-lg font-bold drop-shadow-lg text-center p-4">View Project</p>
                </div>
              </div>
            ))}
          </div>
